...
describe("Kastj Escrow", function () {
  async function deployFixture() {
    const [owner, creator, recipient, supporter, treasury] =
  ...
  it("releases funds with 5% creator reward and 2% platform fee", async () => {
    const { creator, recipient, supporter, treasuryContract, manager } =
      await deployFixture();
    ...
    expect(
      await ethers.provider.getBalance(await treasuryContract.getAddress())
    ).to.equal(ethers.parseEther("2"));
  });

  it("allows refund when proposal fails", async () => {
    const { creator, recipient, supporter, manager, vault } =
      await deployFixture();
  ...
  it("prevents double withdraw", async () => {
    ...
    await expect(vault.connect(supporter).withdraw(1)).to.be.revertedWith(
      "Already withdrawn"
    );
  });
});